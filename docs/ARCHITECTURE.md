...
Главный архитектурный принцип AI Memory OS: любой источник информации проходит один и тот же путь.

```text
Любой источник
  -> Input Normalization
  -> Normalized Text
  -> Natural Intent / LLM Classification
  -> Context
  -> Open Cycle / MemoryItem
  -> Morning Focus
```

Telegram, голосовые сообщения, будущий Voice Pendant, Mini App, документы, ссылки, email и другие интеграции не должны создавать отдельную бизнес-логику памяти. Они только доставляют информацию в общий конвейер.

Это защищает ядро продукта от зависимости от конкретного устройства, сервиса или интерфейса.
...
`DELETE_MEMORY` не выполняется автоматически.

Поток удаления:

```text
пользователь: "удали запись про страуса"
  -> save MemoryItem с самой просьбой удалить
  -> Natural Intent: DELETE_MEMORY
  -> найти похожий OpenCycle / MemoryItem
  -> показать кандидата пользователю
  -> кнопки: "Да, удалить" / "Нет"
  -> удалить только после подтверждения
```

Callback-кнопка содержит id кандидата на удаление. Перед удалением Memory Core проверяет, что запись принадлежит текущему пользователю. Если запись уже удалена или не принадлежит пользователю, удаление не выполняется.
### Voice Input Flow

Voice больше не является отдельной логикой памяти.
...
Telegram voice
  ...
  -> ingestTelegramText
  -> Natural Intent
  -> MemoryItem / OpenCycle / close / delete confirmation
```

...
Команды Telegram остаются как техническая панель MVP, но главный сценарий продукта — обычные человеческие фразы.
...
Бот сохранит запись и создаст открытый цикл.

```text
помидоры уже купил
```

Бот сохранит эту фразу как `MemoryItem`, найдёт похожий открытый цикл `Купить помидоры` и закроет его.

```text
письмо в ЖЭК написал
```

Бот попробует найти открытый цикл `Написать письмо в ЖЭК` и закрыть его.

Важно: удаление и изменение по естественным фразам пока не выполняются автоматически. Бот может понять намерение, но безопасно сохранит фразу без удаления данных. Позже для этого будет добавлено подтверждение.
## Safe Delete Confirmation

Естественные фразы удаления теперь проходят через подтверждение.
...
Если бот находит похожую запись, он отвечает:
...
Удаление выполняется только после нажатия `Да, удалить`. Это защищает память от случайной ошибки LLM.
## Voice Input

Голосовые сообщения теперь проходят через тот же поток, что и текст.
...
```text
[голосом] купить апельсины завтра
```

Бот отвечает примерно так:

```text
🎤 Распознал:
купить апельсины завтра

✅ Запомнил в Inbox.
🧠 Понял как: покупка.
```

Если позже сказать голосом:

```text
апельсины купил
```

бот должен закрыть открытый цикл так же, как при текстовом сообщении.

Для работы нужен `OPENAI_API_KEY`. Модель транскрибации можно поменять через:
